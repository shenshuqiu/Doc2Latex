...
def syntax_interpreter(ldoc: Any, string: str, serial: str) -> None:
    """
    ...
    if "【" not in string:
        ...
        if colon_match:
            ...
            if syntax == "图片":  # 【图片：图片名称】
                ...
                syntax_interpreter(ldoc, pre, serial)
                ...
                ldoc.append(NoEscape(r"\vspace{1ex}"))
                syntax_interpreter(ldoc, after, serial)
                ...
                syntax_interpreter(ldoc, after.replace(BOLD_COLOR, color), serial)
            ...
            raise TypeError(f"{serial}中的语法{content}未定义")